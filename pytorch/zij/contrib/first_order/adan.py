# Adapted from https://github.com/sail-sg/Adan (commit 2c65bea)
# Copyright (c) 2022 Garena Online Private Limited. Licensed under the Apache-2.0 license.
...
class Adan(Optimizer):
    r"""Implements Adan, an adaptive optimizer with Nesterov-style momentum.
    ...
    where :math:`m_t` is the gradient moment, :math:`v_t` is the moment of the
    gradient difference, :math:`n_t` is the second moment of the
    Nesterov-corrected gradient, and :math:`\lambda` is the decoupled weight
    decay. The decay rates :math:`(\beta_1, \beta_2, \beta_3)` are passed as
    ``betas``. When ``no_prox`` is set, the weight decay multiplies
    :math:`\theta_{t-1}` before the gradient step rather than being applied
    proximally.
